...
# Automatically generated by codeaegis init.

# Check if codeaegis is installed
if ! command -v codeaegis >/dev/null 2>&1; then
    echo "⚠️ CodeAegis not found in PATH. Skipping pre-commit scan."
    exit 0
fi

# Extract staged files (excluding deletions)
files=$(git diff --cached --name-only --diff-filter=d)
if [ -z "$files" ]; then
    exit 0
fi

echo "🛡️ CodeAegis: Scanning staged files for vulnerabilities..."

# Scan only the staged files
codeaegis scan $files --severity-threshold high
exit_code=$?

if [ $exit_code -ne 0 ]; then
    echo "❌ Commit rejected. Please fix the security findings above or bypass using 'git commit --no-verify'."
    exit 1
fi
